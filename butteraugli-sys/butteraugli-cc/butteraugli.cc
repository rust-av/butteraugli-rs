...
static inline void CheckImage(const ImageF &image, const char *name) {
	...
	for (size_t y = 0; y < image.ysize(); ++y) {
		const float *const BUTTERAUGLI_RESTRICT
		row = image.Row(y);
...
ImageF Convolution(const ImageF &in,
	...
	for (size_t y = 0; y < in.ysize(); ++y) {
		...
		for (int x = border1; x < border2; ++x) {
			...
			float *const BUTTERAUGLI_RESTRICT
			row_out = out.Row(x);
			float sum = 0.0f;
			for (int j = 0; j < len; ++j) {
...
std::vector <ImageF> OpsinDynamicsImage(const std::vector <ImageF> &rgb) {
	...
	for (size_t y = 0; y < rgb[0].ysize(); ++y) {
		...
		row_b = rgb[2].Row(y);
		const float *const BUTTERAUGLI_RESTRICT
		row_blurred_r = blurred[0].Row(y);
		const float *const BUTTERAUGLI_RESTRICT
		row_blurred_g = blurred[1].Row(y);
		const float *const BUTTERAUGLI_RESTRICT
		row_blurred_b = blurred[2].Row(y);
		float *const BUTTERAUGLI_RESTRICT
		row_out_x = xyb[0].Row(y);
		float *const BUTTERAUGLI_RESTRICT
		row_out_y = xyb[1].Row(y);
		float *const BUTTERAUGLI_RESTRICT
		row_out_b = xyb[2].Row(y);
...
static void SeparateFrequencies(
	...
	for (int i = 0; i < 3; ++i) {
		...
		for (size_t y = 0; y < ysize; ++y) {
			float * BUTTERAUGLI_RESTRICT
			const row_mf = ps.mf[i].Row(y);
			float * BUTTERAUGLI_RESTRICT
			const row_hf = ps.hf[i].Row(y);
			for (size_t x = 0; x < xsize; ++x) {
				row_hf[x] = row_mf[x];
			}
		}
		...
		if (i == 0) {
			for (size_t y = 0; y < ysize; ++y) {
				float * BUTTERAUGLI_RESTRICT
				const row_mf = ps.mf[0].Row(y);
				float * BUTTERAUGLI_RESTRICT
				const row_hf = ps.hf[0].Row(y);
				for (size_t x = 0; x < xsize; ++x) {
					row_hf[x] -= row_mf[x];
			...
			for (size_t y = 0; y < ysize; ++y) {
				float * BUTTERAUGLI_RESTRICT
				const row_mf = ps.mf[1].Row(y);
				float * BUTTERAUGLI_RESTRICT
				const row_hf = ps.hf[1].Row(y);
				for (size_t x = 0; x < xsize; ++x) {
					row_hf[x] -= row_mf[x];
	...
	for (int i = 0; i < 2; ++i) {
		...
		if (i == 0) {
			...
			for (size_t y = 0; y < ysize; ++y) {
				float * BUTTERAUGLI_RESTRICT
				const row_uhf = ps.uhf[1].Row(y);
				float * BUTTERAUGLI_RESTRICT
				const row_hf = ps.hf[1].Row(y);
				float * BUTTERAUGLI_RESTRICT
				const row_lf = ps.lf[1].Row(y);
				for (size_t x = 0; x < xsize; ++x) {
					row_uhf[x] -= row_hf[x];
	...
	for (size_t y = 0; y < ysize; ++y) {
		float * BUTTERAUGLI_RESTRICT
		const row_x = ps.lf[0].Row(y);
		float * BUTTERAUGLI_RESTRICT
		const row_y = ps.lf[1].Row(y);
		float * BUTTERAUGLI_RESTRICT
		const row_b = ps.lf[2].Row(y);
		for (size_t x = 0; x < xsize; ++x) {
...
static void SameNoiseLevels(const ImageF &i0, const ImageF &i1,
	...
	for (size_t y = 0; y < i0.ysize(); ++y) {
		const float *BUTTERAUGLI_RESTRICT
		const row0 = i0.Row(y);
		const float *BUTTERAUGLI_RESTRICT
		const row1 = i1.Row(y);
		float * BUTTERAUGLI_RESTRICT
		const to = blurred.Row(y);
	...
	for (size_t y = 0; y < i0.ysize(); ++y) {
		const float *BUTTERAUGLI_RESTRICT
		const row = blurred.Row(y);
		float * BUTTERAUGLI_RESTRICT
		const row_diff = diffmap->Row(y);
		for (size_t x = 0; x < i0.xsize(); ++x) {
			double diff = row[x];
...
ImageF CalculateDiffmap(const ImageF &diffmap_in) {
	...
	for (size_t y = 0; y < diffmap.ysize(); ++y) {
		const float *const BUTTERAUGLI_RESTRICT
		row_in = diffmap_in.Row(y);
		float *const BUTTERAUGLI_RESTRICT
		row_out = diffmap.Row(y);
...
void MaskPsychoImage(const PsychoImage &pi0, const PsychoImage &pi1,
	...
	for (int i = 0; i < 2; ++i) {
		...
		for (size_t y = 0; y < ysize; ++y) {
			const float *const BUTTERAUGLI_RESTRICT
			row_hf0 = pi0.hf[i].Row(y);
			const float *const BUTTERAUGLI_RESTRICT
			row_hf1 = pi1.hf[i].Row(y);
			const float *const BUTTERAUGLI_RESTRICT
			row_uhf0 = pi0.uhf[i].Row(y);
			const float *const BUTTERAUGLI_RESTRICT
			row_uhf1 = pi1.uhf[i].Row(y);
			float *const BUTTERAUGLI_RESTRICT
			row0 = mask_xyb0[i].Row(y);
			float *const BUTTERAUGLI_RESTRICT
			row1 = mask_xyb1[i].Row(y);
			for (size_t x = 0; x < xsize; ++x) {
...
static void MaltaDiffMapImpl(const ImageF &lum0, const ImageF &lum1,
	...
	for (size_t y = 0, ix = 0; y < ysize_; ++y) {
		const float *BUTTERAUGLI_RESTRICT
		const row0 = lum0.Row(y);
		const float *BUTTERAUGLI_RESTRICT
		const row1 = lum1.Row(y);
...
ImageF DiffPrecompute(const ImageF &xyb0, const ImageF &xyb1) {
	...
	for (size_t y = 0; y < ysize; ++y) {
		...
		row1_in = xyb1.Row(y);
		const float *const BUTTERAUGLI_RESTRICT
		row0_in2 = xyb0.Row(y2);
		const float *const BUTTERAUGLI_RESTRICT
		row1_in2 = xyb1.Row(y2);
		float *const BUTTERAUGLI_RESTRICT
		row_out = result.Row(y);
		for (size_t x = 0; x < xsize; ++x) {
			if (x + 1 < xsize) {
	...
	*rgb = CreatePlanes<uint8_t>(xsize, ysize, 3);
	...
	switch (components) {
		case 1: {
			// GRAYSCALE
			for (int y = 0; y < ysize; ++y) {
				const uint8_t *const BUTTERAUGLI_RESTRICT
				row = row_pointers[y];
				uint8_t *
				const BUTTERAUGLI_RESTRICT row0 = (*rgb)[0].Row(y);
				uint8_t *
				const BUTTERAUGLI_RESTRICT row1 = (*rgb)[1].Row(y);
				uint8_t *
				const BUTTERAUGLI_RESTRICT row2 = (*rgb)[2].Row(y);
		...
		case 2: {
			...
			for (int y = 0; y < ysize; ++y) {
				...
				const BUTTERAUGLI_RESTRICT row2 = (*rgb)[2].Row(y);
				uint8_t *
				const BUTTERAUGLI_RESTRICT row3 = (*rgb)[3].Row(y);
	...
	switch (cinfo.out_color_space) {
		case JCS_GRAYSCALE:
			while (cinfo.output_scanline < cinfo.output_height) {
				...
				row = buffer[0];
				uint8_t *
				const BUTTERAUGLI_RESTRICT row0 =
					(*rgb)[0].Row(cinfo.output_scanline - 1);
				uint8_t *
				const BUTTERAUGLI_RESTRICT row1 =
					(*rgb)[1].Row(cinfo.output_scanline - 1);
				uint8_t *
				const BUTTERAUGLI_RESTRICT row2 =
			...
			for (int y = 0; y < ysize; ++y) {
				...
				for (size_t x = 0; x < xsize; x++) {
					...
					if (row_alpha[x] == 255) {
						...
						value =
							(row_rgb[x] * fg_weight + background * bg_weight +
							 127) / 255;
					}
					row_linear[x] = kSrgbToLinearTable[value];
				}
...
int Run(int argc, char *argv[]) {
	...
	double diff_value;
	if (!ButteraugliInterface(linear1, linear2, 1.0,
	                          diff_map, diff_value)) {
...
double diff_value(char *filename1, char *filename2) {
	...
	std::vector <Image8> rgb1 = ReadImageOrDie(filename1);
	std::vector <Image8> rgb2 = ReadImageOrDie(filename2);
